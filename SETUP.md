...
12. Если Codex успешно завершился без изменений файлов, задача будет помечена как completed и бот покажет финальный ответ без push/PR.
...
Если задача успешно завершилась без изменений файлов, например “залей на тест” или другая run/deploy операция, бот отправит финальный результат со статусом `completed` без approve и без PR.
