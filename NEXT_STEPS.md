...
2. Тестовое покрытие базовых lifecycle-сценариев есть, но нужны integration/mock-based тесты.
...
| Docker isolated execution | Готово | На VPS `WORKER_EXECUTION_MODE=docker` запускает одноразовые worker containers. |
...
2. ✅ Добавлен Docker execution mode для Worker.
...
1. ✅ Подготовлены `.env.example` для Bot/Orchestrator и `.env.vps.example`.
2. ✅ Добавлен production compose для telegram-bot, orchestrator и worker image.
3. ✅ Настроен persistent workspace volume на VPS.
4. ✅ Описана Codex auth внутри VPS runtime.
5. ✅ Добавлен healthcheck для Orchestrator.
6. Добавить systemd unit или отдельную compose restart-инструкцию для автозапуска после reboot.
...
Следующим шагом лучше сделать voice flow, затем route/worker integration tests.
